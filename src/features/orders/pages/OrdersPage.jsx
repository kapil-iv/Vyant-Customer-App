...
export function OrdersPage() {
  ...
  return (
    <section className="space-y-6 max-w-4xl mx-auto py-8">
      <h1 className="text-3xl font-black text-slate-900 border-l-4 border-indigo-600 pl-3">My Orders</h1>
      <div className="flex flex-col gap-4">
        {orders.map((order) => (
          <article key={order._id} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm hover:shadow-md transition-shadow">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div>
                <div className="flex items-center gap-3 mb-1">
                  <p className="font-bold text-lg text-slate-800">Order #{order._id.slice(-8).toUpperCase()}</p>
              ...
              <div className="flex flex-col md:items-end pb-2 md:pb-0 border-b md:border-b-0 border-slate-100 mb-2 md:mb-0">
                <p className="text-sm text-slate-500 font-medium">Total Amount</p>
                <p className="text-xl font-black text-slate-900"><Price value={order.totalAmount ?? order.total ?? 0} /></p>
              </div>
            </div>
