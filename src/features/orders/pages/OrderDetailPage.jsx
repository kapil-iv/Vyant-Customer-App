...
export function OrderDetailPage() {
  ...
  if (!order) return <div className="p-8 text-center text-slate-500 font-medium">Order not found.</div>;

  const itemsTotal = order.items?.reduce((sum, item) => sum + ((item.price || 0) * (item.quantity || 1)), 0) || 0;

  return (
    <section className="mx-auto max-w-5xl space-y-6 py-8 text-slate-900">
      <Link to="/orders" className="flex w-fit items-center gap-1 text-sm font-semibold text-indigo-600 hover:underline">
        &larr; Back to all orders
      </Link>

      <article className="flex flex-col justify-between gap-4 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm md:flex-row md:items-start">
        <div>
          <h1 className="mb-1 text-2xl font-black">Order #{order._id.slice(-8).toUpperCase()}</h1>
          <p className="text-sm font-medium text-slate-500">Placed on {new Date(order.createdAt).toLocaleString()}</p>
        </div>
        <div className="flex flex-col gap-2 md:items-end">
          ...
          {order.trackingStatus ? (
            <p className="text-sm font-semibold capitalize text-slate-600">Tracking: <span className="text-slate-900">{order.trackingStatus.replace("_", " ")}</span></p>
          ) : null}
        </div>
      </article>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        <div className="space-y-6 lg:col-span-2">
          <article className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            <h2 className="mb-4 border-b border-slate-100 pb-3 text-lg font-bold">Items Ordered</h2>
            <div className="space-y-3">
              {order.items?.map((item) => (
                <div key={item._id} className="flex items-center gap-4 rounded-xl border border-slate-100 bg-slate-50/70 p-3 transition hover:bg-slate-50">
                  <img src={item.product?.images?.[0] || "https://via.placeholder.com/150"} alt={item.name} className="h-20 w-20 rounded-lg border border-slate-200 bg-white object-cover shadow-sm" />
          ...
          <article className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            <h2 className="mb-4 border-b border-slate-100 pb-3 text-lg font-bold">Tracking Timeline</h2>
            <OrderTimeline events={timelineData.timeline} trackingStatus={timelineData.trackingStatus || order.trackingStatus} />
            {order.deliveryPartner ? (
              <p className="mt-5 inline-block rounded-lg border border-slate-200 bg-slate-50 p-3 text-sm font-bold text-slate-700 shadow-sm">
        ...
        <div className="space-y-6">
          <article className="rounded-2xl border border-slate-200 bg-slate-50 p-6 shadow-sm">
            <h2 className="mb-4 border-b border-slate-200 pb-3 text-lg font-bold">Payment Summary</h2>
            <div className="mb-4 space-y-3 text-sm font-medium text-slate-600">
              <div className="flex justify-between">
                <span>Subtotal ({order.items?.length || 0} items)</span>
                <span className="text-slate-900"><Price value={itemsTotal} /></span>
              </div>
              {order.platformFee > 0 ? (
                <div className="flex justify-between">
                  <span>Platform Fee</span>
                  <span className="text-slate-900"><Price value={order.platformFee} /></span>
                </div>
              ) : null}
          ...
          {['pending', 'processing'].includes(order.status) ? (
            <article className="rounded-2xl border border-red-100 bg-red-50/80 p-5 shadow-sm">
              ...
              <button
                className="w-full rounded-xl border border-red-200 bg-white px-4 py-3 text-sm font-bold text-red-600 shadow-sm transition-all hover:border-red-300 hover:bg-red-50"
                onClick={async () => {
          ...
          {order.status === "delivered" && order.refundStatus === "none" ? (
            <article className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
              <h2 className="mb-3 text-sm font-bold uppercase tracking-wider text-slate-600">Return Items</h2>
              <ReturnOrderForm
