...
export function CartItem({ item }) {
  ...
  return (
    <div className="flex gap-4 border-b border-slate-100 py-6 group">
      {/* Product Image */}
      <Link to={`/products/${productId}`} className="shrink-0">
        <img
          src={images?.[0] ?? "https://via.placeholder.com/150"}
          alt={name}
          className="h-28 w-24 rounded-xl object-cover border border-slate-100 shadow-sm"
        />
      </Link>
      ...
      <div className="flex flex-1 flex-col">
        ...
        <div className="flex justify-between items-start">
          <div className="text-left">
            <Link to={`/products/${productId}`} className="font-bold text-slate-900 hover:text-blue-600 transition-colors line-clamp-1">
              {name}
            </Link>
            <div className="mt-1 flex flex-wrap gap-2 text-[11px] font-bold uppercase tracking-wider text-slate-400">
              {selectedSize && <span className="bg-slate-100 px-2 py-0.5 rounded">Size: {selectedSize}</span>}
              {selectedColor && <span className="bg-slate-100 px-2 py-0.5 rounded">Color: {selectedColor}</span>}
        ...
        <div className="flex items-center justify-between mt-auto pt-4">
          <div className="flex items-center bg-slate-50 rounded-lg p-1 border border-slate-200">
            <button
              className="p-1 text-slate-600 hover:bg-white hover:shadow-sm rounded-md disabled:opacity-30 transition-all"
            ...
            <span className="px-4 text-sm font-black text-slate-800">{quantity}</span>
